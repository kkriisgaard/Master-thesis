     ...
     )
{
	...
	for(int i=0;i<upper;i+=8) 
	{
		...
		// Ai = _mm_loadu_si128(ad+i*CRYPTO_KEYBYTES); //  Load
	...
	if(fin_ad)
	{
		...
		_2delta = /*a_Delta[0];//*/ mul2(a_Delta[7]); // mul2(a_Delta[0]); // delta -> a_Delta[0]
		delta =  _mm_xor_si128( _mm_xor_si128(a_Delta[7] /*delta*/,_2delta) , mul2(_2delta) );
	}
	else
	{
		Ai = _mm_loadu_si128(ad+upper*CRYPTO_KEYBYTES); 
		delta = mul2(a_Delta[7]); //mul2(a_Delta[0]); 
	}
	AA = _mm_xor_si128( Ai,delta );
	Z = encrypt_block(AA);
	...
	__m128i Mg[8]; 
	/*__m128i Xg[8]; // Actually seems slower with one array (though ThinLinc is not reliable)
	__m128i Yg[8]; 	// To change to a multi array method: Remove the comment before Xg
	__m128i Cg[8]; // */
	__m128i M,X,C,_2W;
	__m128i W = IV;
	__m128i deltaC = L2;	
	delta = L;
	int i,j;
	a_Delta[7] = L; /*COLM is one-indexed by nature...*/
	__m128i a_DeltaC[8];
	a_DeltaC[7] = L2; /*Warning: COLM is one-indexed by nature...*/
	__m128i mes[8]/*, a_W[16]*/;
	...
	for(i=0;i<mf;i+=8)
	{
		...
		//{
			...
			Ml = mes[7]^(Ml^mes[0])^(mes[1]^mes[2])^(mes[3]^mes[4])^(mes[5]^mes[6]); 
			// Ml = _mm_xor_si128( M,Ml );
			// for(j=0;j<8;++j){
			// delta = mul2(delta); 
			// Mg[j] = _mm_xor_si128( mes[j],delta );// }
			...
			Mg[4] = _mm_xor_si128(mes[4],a_Delta[4]);
			Mg[5] = _mm_xor_si128(mes[5],a_Delta[5]);
			Mg[6] = _mm_xor_si128(mes[6],a_Delta[6]);
			Mg[7] = _mm_xor_si128(mes[7],a_Delta[7]);
			
			
			
			 //
		//}
		encrypt_8block2(Mg/*,Xg*/  ); // Change this to encrypt_8block, and remove the commented Xg
		for(j=0;j<8;++j)		
		{
			// _2W = mul2(W);
		...
		encrypt_8block2(Mg/*,Cg*/ ); // Remove the commented Cg, and change Mg to preferred input
		//for(j=0;j<8;++j)
		//{
			a_DeltaC[0] = mul2(a_DeltaC[7]);
			a_DeltaC[1] = mul2(a_DeltaC[0]);
			a_DeltaC[2] = mul2(a_DeltaC[1]);
			a_DeltaC[3] = mul2(a_DeltaC[2]);
			a_DeltaC[4] = mul2(a_DeltaC[3]);
			a_DeltaC[5] = mul2(a_DeltaC[4]);
			a_DeltaC[6] = mul2(a_DeltaC[5]);
			a_DeltaC[7] = mul2(a_DeltaC[6]); // 
			// deltaC = mul2(deltaC);
			...
			mes[4] = _mm_xor_si128(Mg[4],a_DeltaC[4]);
			mes[5] = _mm_xor_si128(Mg[5],a_DeltaC[5]);
			mes[6] = _mm_xor_si128(Mg[6],a_DeltaC[6]);
			mes[7] = _mm_xor_si128(Mg[7],a_DeltaC[7]); // 
