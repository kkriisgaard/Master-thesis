     ...
     )
{
	...
	for(i=0;i<upper;++i) 
	{
		...
		a_Ai = _mm_loadu_si128(ad+(i  )*CRYPTO_KEYBYTES);
			
		
		a_Ai = _mm_xor_si128(a_Ai,delta);

		
		a_Ai = encrypt_block(a_Ai);
		
		// Wp ^= a_Ai[0];
		
		Wp = Wp^a_Ai;// _mm_xor_si128( Z,Wp );		
	}
	if(fin_ad)
	{
	...
	for(i=0;i<mf;++i)
	{
			...
			mes = _mm_loadu_si128(m+((i  )*CRYPTO_KEYBYTES));
			
			
			Ml = (mes^Ml); 
			
			
			Mg = _mm_xor_si128(mes,delta);
		
			
		Mg = encrypt_block(Mg);
		Wp = _mm_xor_si128(Mg,mul2(W));
		Mg = _mm_xor_si128(Wp,W);
		W = Wp;
		...
		// Mg[0] = encrypt_block(Mg[0]); // 
			...
			mes = _mm_xor_si128(Mg,deltaC);
			
			
			// _mm_storeu_si128( (__m128i *)&c[(i  )*CRYPTO_KEYBYTES], mes[0] ); 
			
			_mm_storeu_si128( (__m128i *)&c[(i  )*CRYPTO_KEYBYTES], mes ); 
