...
const int ver = 4; /*That should take care of the fucking segfault...*/
...
void sk_dec(unsigned char* input, const unsigned char* userkey/*, int ver*/)
{
...
// encryption function of Skinny
void sk1_enc(unsigned char* input, const unsigned char* userkey/*, int ver*/) /* Set ver = 4 */ /* Userkey = tweakey*/
{
	unsigned char state[4][4];
	unsigned char keyCells[3][4][4];
	int i;
	// const int ver = 4;
	memset(keyCells, 0, 48);
	// printf("Memset clears\n");
	for(i = 0; i < 16; i++) 
	{
		state[i>>2][i&0x3] = input[i]&0xFF; // To be used
		 // printf("Statestuff clears for iteration %d\n",i);
		  keyCells[0][i>>2][i&0x3] = userkey[i]&0xFF; // To be used
		    // printf("First keycell clears for iteration %d\n",i);
		    //if (versions[ver][1]>=256)
		        keyCells[1][i>>2][i&0x3] = userkey[i+16]&0xFF; // To be used
		       //  printf("Second keycell clears for iteration %d\n",i);
    }
	// printf("Exit first for-loop\n");
    /*#ifdef DEBUG
        fprintf(fic,"ENC - initial state:                 ");display_cipher_state(state,keyCells,ver);fprintf(fic,"\n");
    #endif*/
	for(i = 0; i < 48 /*versions[ver][2]*/; i++){ // Change to number of rounds
        if (versions[ver][0]==64)
            {SubCell4(state);}
        else
            {SubCell8(state);} /*Curly brackets. Always use the damned curly brackets*/
            /*#ifdef DEBUG
            fprintf(fic,"ENC - round %.2i - after SubCell:      ",i);display_cipher_state(state,keyCells,ver);fprintf(fic,"\n");
            #endif*/
       AddConstants(state, i);// printf("AddConst cleas for round %d\n",i);
            /*#ifdef DEBUG
            fprintf(fic,"ENC - round %.2i - after AddConstants: ",i);display_cipher_state(state,keyCells,ver);fprintf(fic,"\n");
            #endif*/
        AddKey(state, keyCells, ver);// printf("AddKey cleas for round %d\n",i);
            /*#ifdef DEBUG
            fprintf(fic,"ENC - round %.2i - after AddKey:       ",i);display_cipher_state(state,keyCells,ver);fprintf(fic,"\n");
            #endif*/
        ShiftRows(state);// printf("ShiftRows cleas for round %d\n",i);
           /*#ifdef DEBUG
            fprintf(fic,"ENC - round %.2i - after ShiftRows:    ",i);display_cipher_state(state,keyCells,ver);fprintf(fic,"\n");
            #endif*/
        MixColumn(state);// printf("MixColumns cleas for round %d\n",i);
            /*#ifdef DEBUG
