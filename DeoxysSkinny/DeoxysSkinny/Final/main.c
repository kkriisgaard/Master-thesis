...
void reset_tweak(const unsigned char at[16], const unsigned long long mlen, u256 *tw_mes){
 ...
 void print_tweak(int len, const u256 *tw)
 {
  ...
  const unsigned char *k, const unsigned char *ad, unsigned long long adlen, const unsigned char nonce[15],unsigned char at[16], u256 *tweak_pre, u256 *tweak_pre_ad) /* This may literally be the dumbest way, excluding offensively stupid ways, I could do this, but it has two advantages: 1) I don't fuck with the Skinny internals. 2) It's simple.*/
  {
	  ...
	  unsigned long long ad_ext = 0x2000000000000000000000;
	  unsigned long long zero = 0;
	  unsigned char tk_1s[32];
	  unsigned char ext_tweak[32]; // probably not necessary
	  unsigned char tmp_ar[16];
	  unsigned char n[16];
	  int off_fin = adlen/1024; // I wonder if it saves the remainer of the integer division if called upon for later (e.g. right below)
	  int num_fin = adlen%1024;
	  memcpy(n+ 1, nonce , 15);
	  ...
	  if(num_fin){
		  memcpy(ext_tweak,k,16);
		  memcpy(ext_tweak+16,ad_ext,8);
		  memcpy(ext_tweak+24,zero,8);
		  ptr = &ext_tweak[24];
		 
		  for(i=off_fin;i<off_fin+num_fin;++i){
			memcpy(tmp_ar, ad + i*16, 16);
			(*ptr) = i;  
			sk1_enc(tmp_ar,ext_tweak);
			ptr2 = &auth[0];
			ptr3 = &tmp_ar[0];
			(*ptr2) ^= (*ptr3);
			ptr2 = &auth[8];
	  ...
	  reset_tweak();
	  #endif
	  off_fin = inlen/1024; // I wonder if it saves the remainer of the integer division if called upon for later (e.g. right below)
	  num_fin = inlen%1024;
	  ...
	  if(num_fin){
		  memcpy(ext_tweak+16,zero,8);
		  memcpy(ext_tweak+24,zero,8);
		  ptr = &ext_tweak[24];
		 
		  for(i=off_fin;i<off_fin+num_fin;++i){
			memcpy(tmp_ar, in + i*16, 16);
			(*ptr) = i;  
			sk1_enc(tmp_ar,ext_tweak);
			ptr2 = &auth[0];
			ptr3 = &tmp_ar[0];
...
// Encryption	
  ...
  const unsigned char *k, const unsigned char *ad, unsigned long long adlen, const unsigned char nonce[15],unsigned char at[16], u256 *tweak_pre, u256 *tweak_pre_ad) /* This may literally be the dumbest way, excluding offensively stupid ways, I could do this, but it has two advantages: 1) I don't fuck with the Skinny internals. 2) It's simple.*/
  {
	  ...
	  unsigned char auth[16] = {0x00,0x00,0x00,0x00,  0x00,0x00,0x00,0x00  ,0x00,0x00,0x00,0x00,  0x00,0x00,0x00,0x00};
	  u256 x[32];
	  unsigned long long *ptr,*ptr2,*ptr3,*ptr4;
	  unsigned char tk_1s[32];
	  unsigned char n[16];
	  unsigned char ext_tweak[32]; // probably not necessary
	  unsigned char tmp_ar[16];
	  int off_fin = inlen/1024; 
	  int num_fin = inlen%1024;
	  n[0] = 0x00;
	  memcpy(n+ 1, nonce , 15);
