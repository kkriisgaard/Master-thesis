...
/* #define ENCR_8_BLOCKS(message) \
			...
			message[0] = _mm_xor_si128(message[0],tmp); \
			message[1] = _mm_xor_si128(message[1],_mm_xor_si128(tmp,z_s[0][0]) ); \
			message[2] = _mm_xor_si128(message[2],_mm_xor_si128(tmp,z_s[0][1]) ); \
			message[3] = _mm_xor_si128(message[3],_mm_xor_si128(tmp,z_s[0][2]) ); \
			message[4] = _mm_xor_si128(message[4],_mm_xor_si128(tmp,z_s[0][3]) ); \
			message[5] = _mm_xor_si128(message[5],_mm_xor_si128(tmp,z_s[0][4]) ); \
			message[6] = _mm_xor_si128(message[6],_mm_xor_si128(tmp,z_s[0][5]) ); \
			message[7] = _mm_xor_si128(message[7],_mm_xor_si128(tmp,z_s[0][6]) ); \
			...
			message[0] = _mm_aesenc_si128(message[0],tmp); \
			message[1] = _mm_aesenc_si128(message[1],xx[0] ); \
			message[2] = _mm_aesenc_si128(message[2],xx[1] ); \
			message[3] = _mm_aesenc_si128(message[3],xx[2] ); \
			message[4] = _mm_aesenc_si128(message[4],xx[3] ); \
			message[5] = _mm_aesenc_si128(message[5],xx[4] ); \
			message[6] = _mm_aesenc_si128(message[6],xx[5] ); \
			message[7] = _mm_aesenc_si128(message[7],xx[6] ); \
     ...
     )
{
	...
	for(i=0;i<numblocks_mes;i+=8) 
	{
		...
		if(z==31)
		{
			// printf("i = %d\nShould be at blocks starting at %d",i,i*CRYPTO_KEYBYTES);
			z=0;
			tweaks[0] = _mm_shuffle_epi8(tweaks[0],prop_mask);
			tweaks[0] = _mm_add_epi8(tweaks[0],prop);
			tweaks[1] = _mm_shuffle_epi8(tweaks[1],h);
	...
	tag = _mm_aesenc_si128(tag,xx[6]);
	
	
	xx[0] = _mm_xor_si128(keys[8],tweaks[0]);
	tag = _mm_aesenc_si128(tag,xx[0]);
	
	xx[1] = _mm_xor_si128(keys[9],tweaks[1]);
	xx[2] = _mm_xor_si128(keys[10],tweaks[2]);
	xx[3] = _mm_xor_si128(keys[11],tweaks[3]);
	xx[4] = _mm_xor_si128(keys[12],tweaks[4]);
	xx[5] = _mm_xor_si128(keys[13],tweaks[5]);
	xx[6] = _mm_xor_si128(keys[14],tweaks[6]);
	...
	if(fin_mes) // Not sure what to do in this case. Until further notice, I'll just do 10* padding.
	{
		...
		tmp = _mm_xor_si128(keys[1],tweaks[1]); /* Test reorganizing this stuff*/
		idx = _mm_aesenc_si128(idx,tmp);
		tmp = _mm_xor_si128(keys[2],tweaks[2]);
		idx = _mm_aesenc_si128(idx,tmp);
		tmp = _mm_xor_si128(keys[3],tweaks[3]);
